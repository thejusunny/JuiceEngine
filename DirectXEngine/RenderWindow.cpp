...
bool RenderWindow::Initialize(WindowContainer * pWindowContainer, HINSTANCE hInstance, std::string window_title, std::string window_class, int width, int height)
{
	...
	int centerX = GetSystemMetrics(SM_CXSCREEN) /2 - (this->width / 2);
	int centerY = GetSystemMetrics(SM_CYSCREEN) /2 - (this->height / 2);
	RECT wr;
	wr.left = centerX;
	wr.top = centerY;
...
HWND RenderWindow::GetHWND()const { return this->handle; }
